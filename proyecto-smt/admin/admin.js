...
// ============================================
// GESTI"N DE SESI"N - CORREGIDA
// ============================================

function checkAdminSession() {
    adminToken = sessionStorage.getItem('smt_token');
    const userData = sessionStorage.getItem('smt_user');
...
function saveAdminSession(token, user) {
    sessionStorage.setItem('smt_token', token);
    sessionStorage.setItem('smt_user', JSON.stringify(user));
    adminToken = token;
    adminUser = user;
}
...
function clearAdminSession() {
    ...
    sessionStorage.removeItem('smt_user');
    adminToken = null;
    adminUser = null;
...
async function handleAdminLogin(event) {
    ...
    try {
        ...
        if (result.success) {
            ...
            // ESPERAR UN MOMENTO ANTES DE GUARDAR NUEVA SESI"N
...
function getActionButtons(reporteId, rol) {
        ...
        return `
            <div class="flex gap-4 flex-wrap">
                <button onclick="viewReportSimple('${reporteId}')" 
                    ...
                    <i class="fas fa-eye mr-2"></i>Ver Información Completa
                </button>
    ...
    if (rol === 'JURIDICO') {
        // JURÍDICO: Modal simple sin timeline + opciones de validación
        return `
            <div class="space-y-4">
                <div class="flex gap-4">
                    <button onclick="viewReportSimple('${reporteId}')" 
...
function displayAdminModal(reporte) {
    ...
    const estados = [
        { key: 'NUEVO', label: 'Reporte Recibido', icon: 'fa-inbox', hex: '#3b82f6' },
        { key: 'EN_REVISION_1', label: 'Revisión Jurídica', icon: 'fa-balance-scale', hex: '#eab308' },
        { key: 'VALIDO', label: 'Validado', icon: 'fa-check-circle', hex: '#22c55e' },
        { key: 'SANCION_ASIGNADA', label: 'Sanción Asignada', icon: 'fa-gavel', hex: '#a855f7' },
        { key: 'CERRADO', label: 'Caso Cerrado', icon: 'fa-flag-checkered', hex: '#6b7280' }
    ];
        ...
        return `
            <div class="flex items-start gap-5">
                <div class="flex flex-col items-center">
                    <div class="w-16 h-16 rounded-2xl flex items-center justify-center transition-all shadow-xl ${isCurrent ? 'scale-110' : ''}"
                         style="${isPast
                            ? `background: linear-gradient(135deg, ${estado.hex}, ${estado.hex}cc);${isCurrent ? ` box-shadow: 0 0 0 4px ${estado.hex}55;` : ''}`
                            : 'background: #e5e7eb;'}">
                        <i class="fas ${estado.icon} text-3xl ${isPast ? 'text-white' : 'text-gray-400'}"></i>
                    </div>
                    ${index < estados.length - 1 ? `
                        <div class="w-1 h-20 rounded-full" style="background: ${isPast ? estado.hex + '80' : '#e5e7eb'};"></div>
                    ` : ''}
                </div>
                <div class="flex-1 ${isPast ? '' : 'opacity-40'}">
                    ...
                    ${fechaEstado ? `
                        <p class="text-base font-bold mb-1" style="color: ${estado.hex};">
...
function initAdminModalMap(lat, lng) {
    ...
    try {
        ...
        L.marker([lat, lng], { icon: redIcon }).addTo(adminModalMap)
            .bindPopup('<b>Y" Ubicación del Incidente</b>')
            .openPopup();
...
// ============================================
// MODAL SIMPLE - NOTIFICADOR Y JURÍDICO
// ============================================
...
async function notificarJuridico(reporteId, notificar) {
    ...
    try {
        ...
        if (result.success) {
            showToast(notificar ? 'o" Jurídico ha sido notificado exitosamente' : 'Reporte marcado como NO notificar', 'success');
            closeSimpleModal();
            loadAdminData();
        } else {
...
async function validarReporte(reporteId, esValido) {
    ...
    if (esValido) {
        ...
        if (!sancion) {
            showToast('s️ Debes especificar la sanción que se aplicará', 'error');
            return;
        }
    ...
    try {
        ...
        if (result.success) {
            showToast(esValido ? 'o" Reporte validado y sanción asignada exitosamente' : 'Reporte marcado como NO válido', 'success');
            closeSimpleModal();
            loadAdminData();
        } else {
...
function getRoleLabel(rol) {
    const labels = {
        'ENLACE_GENERAL': 'Y'" Supervisor General',
        'NOTIFICADOR': 'Y" Notificador - Primera Revisión',
        'JURIDICO': 's-️ Jurídico - Validación y Sanciones'
    };
...
function getEstadoClass(estado) {
    const classes = {
        ...
        'RECHAZADO': 'bg-gradient-to-r from-red-500 to-rose-500 text-white'
    };
...
function getEstadoLabel(estado) {
    const labels = {
        'NUEVO': 'Y?. Nuevo',
        'EN_REVISION_1': 'Y" En Revisión',
        'VALIDO': 'o. Validado',
        'SANCION_ASIGNADA': 's-️ Sancionado',
        'CERRADO': 'Y"' Cerrado',
        'RECHAZADO': 'O Rechazado'
    };
...
function getCategoriaLabel(categoria) {
    const labels = {
        'CONDUCTOR': 'Y'...o^️ Conductor',
        'UNIDAD': 'YsO Unidad/Vehículo',
        'SERVICIO': 'Y Servicio',
        'DOCUMENTACION': 'Y"" Documentación',
        'RUTA': 'Y>️ Ruta',
        'OTRO': 'Y"< Otro'
    };
